  ...
  $(function () {
    ...
    function slideSlide(event, ui) {
      ...
      if (ui.values[0] === ui.values[1]) {
        ...
        $('#stageRangeText').text(
          'С ' + ui.values[0] + ' по ' + ui.values[1] + ' этаж'
        );
      }
    }
    ...
    function slideChange(event, ui) {
      ...
      if (ui.values[0] === ui.values[1]) {
        ...
        $button
          .find('.b-dropdown-text')
          .text(ui.values[0] + '-' + ui.values[1] + ' этаж');
      }
    ...
    if (
      $('#stageRangeSlider').slider('values', 0) ===
      $('#stageRangeSlider').slider('values', 1)
    ) {
      $('#stageRangeText').text(
        $('#stageRangeSlider').slider('values', 0) + ' этаж'
      );
    } else {
      $('#stageRangeText').text(
        'С ' +
        ...
        $.ajax({
          ...
          beforeSend: function () {
            ...
            window.history.replaceState(
              {},
              '',
              String(window.location).substring(
                0,
      ...
      if ($elem.length) {
        switch ($elem.attr('type')) {
          case 'radio':
            if (query[key]) {
              $elem
                .closest('.b-filter__menu-item, .b-filter__sort-item')
